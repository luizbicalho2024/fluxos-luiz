...
function Find-Zip {
    ...
    foreach($location in $locations) {
        ...
        $zip=Get-ChildItem -Path $location -File -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -like 'fluxos-luiz-paridade-completa*.zip' -or $_.Name -like 'fluxos-luiz*v4.1.1*.zip' } |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1
        if ($zip) { return $zip.FullName }
    }
    throw @"
Nao encontrei o ZIP da versao 4.1.1.
Baixe 'fluxos-luiz-paridade-completa-v4.1.1.zip' e deixe em Downloads.
"@
}
...
Step '4/10 - Preservando configuracao local e aplicando 4.1.1'
